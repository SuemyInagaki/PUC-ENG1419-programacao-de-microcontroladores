...
def main():
    ...
    print(agora.strftime("São %H:%M do dia %d/%m/%Y"))
    
    #texto = "São " + str(agora.hours) + ":" +  str(agora.minutes)+ " do dia "+ str(agorra.day) + "/"+ str(agora.month) + "/" + str(agora.year)
    #print(texto)    
    ...
    futuro = agora + timedelta(weeks=40)
    print(futuro.strftime("Parabéns! Você terá um bebê no dia %d/%m!"))
    ...
    #campainha.beep(on_time=1, off_time=2, n=5)
    ...
    distancia_em_metros = distancia_em_metros*100
    lcd.message(str(distancia_em_metros))
    ...
    DistanceSensor.threshold_distance = 0.5
    ...
    sensor_de_distancia.threshold = 0.12
    ...
    def nova_prox():
        campainha.beep(on_time=0.2,off_time=0.2)
        
    sensor_de_distancia.when_in_range = nova_prox
    
    sensor_de_distancia.when_out_of_range = led.toggle()
